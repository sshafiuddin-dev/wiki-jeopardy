...
Welcome to **Jeopardy Unleashed** — an AI-powered multiplayer trivia game inspired by the classic Jeopardy format. Questions are generated live by **Llama 3.3 70B via the Groq API** — every board is unique. No sign-up required. Just open, set up, and play!

---

## 🚀 Getting Started

1. Open the game at [sshafiuddin-dev.github.io/jeopardy](https://sshafiuddin-dev.github.io/jeopardy/)
...
### Step 1 — The Board
After clicking **"Create Board"**, a **5×5 grid** appears with 5 categories across the top and 5 point values down the side (e.g. 100, 200, 300, 400, 500). Each cell is a clue worth that many points. All 25 questions are freshly generated by the AI for your chosen topics.
...
If a question seems unclear, repeated, or off-topic:
- Click the **🔄 Change Question** button in the Question Box.
- The AI (Groq / Llama 3.3 70B) fetches a **fresh replacement question** for that clue only — no need to rebuild the whole board.
...
- Topics can be **anything** — the AI adapts to any subject, from niche hobbies to academic topics.
...
- Questions are generated fresh each game — no two boards are the same.
...
2. AI generates 25 unique questions via Groq / Llama 3.3 70B
3. Active player clicks a clue on the board
4. Read the clue → Select your answer within 90 seconds
5. Correct = full points | Wrong = 0 | Pass = −half points
6. Turn rotates → repeat until all 25 clues are done
7. Highest score wins!
```

---

*Built with ❤️ using Groq AI + Vanilla JS. Fork it, deploy it, make it yours!*

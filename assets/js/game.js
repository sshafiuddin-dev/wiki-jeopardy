// ── Core game logic ──
import { state, setStartCooldown, setRefreshCooldown, startCooldown, refreshCooldown } from './state.js';
import { SFX }          from './audio.js';
import { startTimer, stopTimer } from './timer.js';
import { pts, renderPlayers, renderScoreboard, renderBoard, renderOptions,
         markOptions, disableAllOptions, syncActiveQuestionUI,
         showResults, renderHistory, updateCountLabel }   from './render.js';
import { buildBoard, regenerateSingleQuestion, parseTopics,
         ALLOWED_DIFFICULTY, esc }                         from './llm.js';
...
function playerLabel(i, team)  { return team ? `Team ${i+1}` : `Player ${i+1}`; }
function nextAfter(i)          { return (i + 1) % state.players.length; }
...
function resolveTurnAfterClue(){ if (state.active) setTurn(nextAfter(state.active.originTurn)); }

function logHistory(entry) { state.history.push(entry); renderHistory(); }
...
export function openQ(cat, clue, tile, rowIndex) {
  ...
  $('timerBtns').classList.remove('hidden');          // FIX: classList instead of style.display
...
export async function refreshActiveQuestion() {
  if (!state.active || state.active.attemptedBy.length || refreshCooldown) return;  // FIX: use imported refreshCooldown
  ...
  try {
    ...
    const replacement = await regenerateSingleQuestion(state.board, state.active.cat, state.active.clue, difficulty);
    ...
    setTimeout(() => {
      ...
      if (state.active && !state.active.attemptedBy.length) { btn.disabled = false; btn.textContent = '🔄 Change Question'; }
    }, COOLDOWN_MS);
  } catch (e) {
    $('feedback').textContent = `Could not refresh question: ${esc(e.message)}`;
    setRefreshCooldown(false);
    btn.disabled = false; btn.textContent = '🔄 Change Question';
  }
}
...
export function submitAnswer(selected) {
  ...
  if (selected === clue.answer) {
    clue.used = true; tile.disabled = true; state.answered++; state.correct++;
    state.score[state.turn]   = (state.score[state.turn]   || 0) + clue.value;
    state.qCorrect[state.turn]= (state.qCorrect[state.turn]|| 0) + 1;
    markOptions(selected, clue.answer); SFX.correct();
    $('feedback').textContent = `✅ Correct! ${currentLabel()} earns ${pts(clue.value)}.`;
    logHistory({ category:state.active.cat, question:clue.clue, correctAnswer:clue.answer,
      answeredBy:currentLabel(), wasPass:isPass, result:'correct', points:clue.value, origPoints:clue._origValue||clue.value });
    state.active.resolved = true; resolveTurnAfterClue();
    if (state.answered >= state.total) { SFX.roundEnd(); showResults(state.players, state.score); }
    disableAllOptions();
    $('timerBtns').classList.add('hidden');             // FIX: classList instead of style.display
    return;
  }

  markOptions(selected, clue.answer); SFX.wrong();
  ...
  if (attemptedBy.length >= state.players.length) {
    clue.used = true; tile.disabled = true; state.answered++;
    $('feedback').textContent = `No one got it. Correct answer: ${clue.answer}.`;
    logHistory({ category:state.active.cat, question:clue.clue, correctAnswer:clue.answer,
      answeredBy:null, wasPass:false, result:'nobody', points:0, origPoints:clue._origValue||clue.value });
    state.active.resolved = true; resolveTurnAfterClue();
    if (state.answered >= state.total) { SFX.roundEnd(); showResults(state.players, state.score); }
    disableAllOptions();
    $('timerBtns').classList.add('hidden');             // FIX: classList instead of style.display
  ...
  nextTurn();
  $('money').textContent        = pts(clue.value);
  $('activePlayer').textContent = `${currentLabel()}'s turn`;
  $('feedback').textContent     = `❌ Wrong. Next player/team tries for ${pts(clue.value)}.`;
...
export function handlePass() {
  ...
  $('money').textContent        = pts(clue.value);
  $('activePlayer').textContent = `${currentLabel()}'s turn`;
  $('feedback').textContent     = `Passed. Next player/team tries for ${pts(clue.value)}.`;
  renderOptions(clue, submitAnswer, handlePass);
  startTimer(PASS_TIME, handlePass);
}
...
export async function start() {
  if (startCooldown) return;                           // FIX: use imported startCooldown
  const topics = parseTopics($('topic').value);
  const rawN   = Number($('players').value || 2);
  if (rawN < 2 || rawN > 8 || !Number.isInteger(rawN)) {
    $('stateMsg').textContent = '⚠️ Player count must be a whole number between 2 and 8.'; return;
  }
  const difficulty = ALLOWED_DIFFICULTY.includes($('difficulty').value) ? $('difficulty').value : 'mixed';
  ...
  state.history  = [];
  state.turn = 0; state.answered = 0; state.correct = 0; state.total = 25; state.board = []; state.active = null;
  $('questionBox').classList.remove('open'); $('board').innerHTML = ''; $('feedback').textContent = '';
  ...
  try {
    ...
    $('stateMsg').textContent = ''; $('stateMsg').appendChild(msg);
    $('statusPill').textContent = 'Error';
  } finally {
    setStartCooldown(true);
    $('startBtn').disabled = true; $('startBtn').textContent = 'Please wait...';
    setTimeout(() => { setStartCooldown(false); $('startBtn').disabled = false; $('startBtn').textContent = 'Create board'; }, COOLDOWN_MS);
  }
}
...
export function reset() {
  ...
  state.qPassed  = state.players.map(() => 0);
  state.history  = [];
  state.turn = 0; state.answered = 0; state.correct = 0; state.board = []; state.active = null;
  stopTimer();
  $('questionBox').classList.remove('open'); $('board').innerHTML = ''; $('feedback').textContent = '';
  $('refreshQuestionBtn').classList.add('hidden');     // FIX: classList instead of style.display
  renderHistory(); renderPlayers(); renderScoreboard();

// ── Core game logic ──
import { state, setStartCooldown, setRefreshCooldown, refreshCooldown, startCooldown } from './state.js';
...
export async function refreshActiveQuestion() {
  if (!state.active || state.active.attemptedBy.length || refreshCooldown) return;
...
export function submitAnswer(selected) {
  ...
  if (selected === clue.answer) {
    ...
    disableAllOptions(); $('timerBtns').style.display = 'none';
    return;
  }
...
export async function start() {
  ...
  try {
    ...
    const msg = document.createElement('span'); msg.style.color = 'var(--bad)'; msg.textContent = e.message;

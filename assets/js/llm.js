...
const ALLOWED_DIFFICULTY = ['easy', 'mixed', 'hard'];

export { ALLOWED_DIFFICULTY };
...
// ── Generate questions for one category from LLM ──
export async function generateCategoryQuestions(topic, difficulty, count = 5, excludeQuestions = []) {
  ...
  catch (e) { throw new Error('LLM returned invalid JSON'); }
...
// ── Normalize a raw LLM question object into a board clue ──
...
// ── Regenerate a single question (for refresh button) ──
...
export async function buildBoard(topicNames, difficulty) {
  ...
  for (const topic of topicNames.slice(0, 5)) {
    ...
    try { qs = await generateCategoryQuestions(topic, difficulty, 5); }
      ...
      console.warn(`[Board] LLM failed for "${topic}", using placeholder:`, e.message);
      qs = [];
    }
    while (qs.length < 5) {
      const n = qs.length + 1;
    ...
    const clues = qs.slice(0, 5).map((item, r) => normalizeGeneratedQuestion(item, topic, valuePlan[r] || valuePlan[0]));
    categories.push({ name: topic, clues });
  }
  return categories;
}

// ── Topic input sanitizer + parser ──

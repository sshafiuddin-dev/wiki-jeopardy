...
export function updateCountLabel() {
  const team = $('teamMode').checked;
  $('playerCountLabel').textContent = team ? 'Number of teams' : 'Number of players';
  $('playerCountHint').textContent  = team ? 'Use this as the total number of teams.' : 'Use this as the total number of players.';
}
...
export function renderPlayers() {
  ...
  state.players.forEach((p, i) => {
    ...
    d.innerHTML = `<strong>${esc(p)}</strong><span>${pts(state.score[i] || 0)}</span>`;
...
export function renderLeaderboard() {
  ...
  [...state.players.map((n,i) => ({ n, s:state.score[i]||0, q:state.qAsked[i]||0, c:state.qCorrect[i]||0, p:state.qPassed[i]||0, i }))]
    .sort((a,b) => b.s - a.s)
    .forEach((pl,r) => {
      ...
      row.innerHTML = `
        ...
        <span class="pts" style="text-align:right">${pts(pl.s)}</span>
...
export function renderScoreboard() {
  ...
  $('leader').textContent  = li > -1 ? `${state.players[li]} (${pts(state.score[li])})` : '—';
  ...
  $('stateMsg').textContent   = state.total - state.answered > 0
    ? `${state.total - state.answered} clues left` : 'All clues answered';
  renderLeaderboard();
}

export function renderHistory() {
  const c = $('historyContainer');
  if (!state.history.length) { c.innerHTML = '<p class="no-history">No questions answered yet.</p>'; return; }
  let html = `<div style="overflow-x:auto"><table class="history-table">
    <thead><tr><th>#</th><th>Category</th><th>Orig Pts</th><th>Pts Earned</th>
    <th>Question</th><th>Correct Answer</th><th>Answered By</th><th>Type</th><th>Result</th></tr></thead><tbody>`;
  [...state.history].reverse().forEach((h, idx) => {
    const num = state.history.length - idx;
    const typeBadge   = h.wasPass ? `<span class="hist-pass-badge">Pass</span>` : `<span class="hist-orig-badge">Original</span>`;
    const resultCell  = h.result === 'correct' ? `<span class="hist-correct">✅ Correct</span>` : h.result === 'wrong' ? `<span class="hist-wrong">❌ Wrong</span>` : `<span style="color:var(--muted)">— No one</span>`;
    const earnedPts   = h.result === 'correct' ? `<span style="font-weight:800;color:var(--good)">+${h.points}</span>` : `<span style="color:var(--muted)">—</span>`;
    const origPtsCell = h.origPoints !== h.points && h.result === 'correct' ? `<span style="text-decoration:line-through;color:var(--muted);margin-right:4px">${h.origPoints}</span>` : `${h.origPoints}`;
    html += `<tr>
      ...
      <td>${typeBadge}</td><td>${resultCell}</td></tr>`;
  });
...
export function renderOptions(clue, onSubmit, onPass) {
  ...
  clue.options.forEach(opt => {
    ...
    b.className = 'answer'; b.textContent = opt; b.dataset.answer = opt;
    b.onclick = () => onSubmit(opt);
    $('answers').appendChild(b);
  });
  const pb = document.createElement('button');
  pb.className = 'answer'; pb.style.gridColumn = '1 / -1';
  pb.textContent = '❌ Pass to next player/team'; pb.onclick = onPass;
...
export function syncActiveQuestionUI(active, currentPlayerLabel, onSubmit, onPass) {
  ...
  $('refreshQuestionBtn').title         = canRefresh ? 'Replace only this question' : 'Unavailable after an attempt has been made';
}
...
export function renderBoard(board, onTileClick) {
  ...
  for (let r = 0; r < rows; r++) {
    board.forEach(cat => {
      const clue = cat.clues[r]; if (!clue) return;
      const b = document.createElement('button');
      b.className = 'tile'; b.textContent = pts(clue.value); b.disabled = clue.used;
...
export function showResults(players, scores) {
  ...
  ranked.forEach((p,i) => {
    ...
    row.innerHTML = `<span>${['🥇','🥈','🥉'][i]||`${i+1}.`} ${esc(p.name)}</span><span>${pts(p.score)}</span>`;
